...
| function | returns |
|---|---|
| `projects()` | one row per mirrored project (release, tables, cases, files and bytes read) |
...
- **Copy-number samples table.** `copy_number_samples()` describes the tumor/normal pair each file was called on, and its `sample_type` is the pair's first aliquot, not the matrix column. Every matrix column is the tumor aliquot (in TCGA-BLCA all 392 ASCAT3 columns are primary tumor barcodes, while 195 rows say Blood Derived Normal). Use the column barcode or `case_submitter_id`, not that `sample_type`.
...
- **Compressed DICOM needs a decoder.** Slide levels are JPEG baseline and tomosynthesis is JPEG 2000; without a pydicom plugin `read_sample()` raises `NciError` with the install hint. The `dicom` extra installs them.
- **One collection has no sample**: `b_mode_and_ceus_liver` (120 US series); `samples()` is empty and `read_sample()` raises `ValueError`.
- **GDC projects still being added.** 36 of the planned 57 are built; `gdc.projects()` is the source of truth, and an unknown id raises `NotMirroredError` listing the mirrored ones.
